...
class TMBubbleMask: NSObject, TMMask {
	...
	func clippingPathWithRect(_ bounds : CGRect) -> CGPath
	{
		...
		path.addArc(withCenter: CGPoint(x: relativeRect.midX - relativeRect.width*scaleFactor  ,y: relativeRect.midY), radius: relativeRect.width/2 - relativeRect.width*scaleFactor*2, startAngle: TMHeartMask.toRadians(0), endAngle: TMHeartMask.toRadians(100), clockwise: false)
		
		path.addQuadCurve(to: CGPoint(x: relativeRect.width - relativeRect.width*scaleFactor*2, y: relativeRect.midY + relativeRect.width/2 - relativeRect.width*scaleFactor*2), controlPoint: CGPoint(x: relativeRect.width/2, y: relativeRect.height - relativeRect.width*controlScaleFactor))
		path.addQuadCurve(to: CGPoint(x: relativeRect.width - relativeRect.width*scaleFactor*3, y: relativeRect.midY) , controlPoint: CGPoint(x: relativeRect.width/2 + relativeRect.width*controlScaleFactor*2, y: relativeRect.height - relativeRect.width*controlScaleFactor*1.5))
		path.close()
